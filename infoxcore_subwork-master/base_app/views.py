...
def TLdashboard(request):
    if 'tlid' in request.session:
        ...
        if confirm_sal > 0 :
            evnt = Event.objects.filter(start_time__range=(prev_mnth_fst_day,prev_mnth_lst_day)).count()
            print(evnt)
            prev_working_days = ((prev_mnth_lst_day - prev_mnth_fst_day).days - evnt) + 1
            print(prev_working_days)
            salry_per_day_of_prev = round((confirm_sal/prev_working_days), 2)
            print(salry_per_day_of_prev)
